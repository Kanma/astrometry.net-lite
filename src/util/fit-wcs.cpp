...
int fit_sip_wcs(const double* starxyz,
    ...
    for (i=0; i<M; i++) {
        ...
        b1(ngood) = weight * rad2deg(x);
        b2(ngood) = weight * rad2deg(y);
        ...
        for (order=0; order<=sip_order; order++) {
            for (q=0; q<=order; q++) {
                ...
                mA(ngood, j) = weight * pow(u, (double)p) * pow(v, (double)q);
                j++;
            }
        }
        assert(j == N);
        ...
        assert(mA(i, 0) == 1.0 * weight);
        assert(fabs(mA(i, 1) - u * weight) < 1e-12);
        assert(fabs(mA(i, 2) - v * weight) < 1e-12);
    ...
    if (ngood < M) {
        Eigen::HouseholderQR<Eigen::MatrixXd> qr(mA.block(0, 0, ngood, N));
        x1 = qr.solve(b1.head(ngood));
        x2 = qr.solve(b2.head(ngood));
    } else {
        // Solve the equation.
        Eigen::HouseholderQR<Eigen::MatrixXd> qr(mA);
        x1 = qr.solve(b1);
        x2 = qr.solve(b2);
    }
    ...
    if (doshift) {
        // Grab CD.
        sipout->wcstan.cd[0][0] = x1(1);
        sipout->wcstan.cd[0][1] = x1(2);
        sipout->wcstan.cd[1][0] = x2(1);
        sipout->wcstan.cd[1][1] = x2(2);
...
int fit_tan_wcs_solve(const double* starxyz,
    ...
    // -run SVD
    Eigen::Matrix2d mcov = Eigen::Map<Eigen::Matrix<double,2,2,Eigen::RowMajor> >(cov);
    Eigen::JacobiSVD<Eigen::Matrix2d> svd(mcov, Eigen::ComputeFullU | Eigen::ComputeFullV);
    ...
    tanout->cd[0][0] = R(0, 0) * scale; // CD1_1
    tanout->cd[0][1] = R(0, 1) * scale; // CD1_2
    tanout->cd[1][0] = R(1, 0) * scale; // CD2_1
    tanout->cd[1][1] = R(1, 1) * scale; // CD2_2
